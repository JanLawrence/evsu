...
class Inbox_model extends CI_Model{
    ...
    public function getTeacherInbox(){
        ...
        $this->db->select('g.*, sub.subject_name, CONCAT(stud.first_name," ",stud.middle_name," ",stud.last_name) stud_name')
          ...
          ->join('tbl_student_guardian sg', 'sg.student_id = ts.student_id', 'inner')
          ->join('tbl_guardian g', 'g.id = sg.guardian_id', 'inner')
          ->join('tbl_teacher_subjects ss', 'ss.teacher_id = t.id', 'inner')
          ->join('tbl_subject sub', 'sub.id = ss.subject_id', 'inner')
          ->join('tbl_students stud', 'stud.id = ts.student_id', 'inner')
    ...
    public function getParentInbox(){
        ...
        $this->db->select('t.*, sub.subject_name')
          ->from('tbl_guardian g')
          ->join('tbl_student_guardian sg', 'g.id = sg.guardian_id', 'inner')
          ->join('tbl_teacher_student ts', 'ts.student_id = sg.student_id', 'inner')
